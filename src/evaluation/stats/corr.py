...
def main(model_id, datafile, outputpath):
    ...
    line=";".join([str(x) for x in [model_id, os.path.basename(datafile), pearsonr, pearson_pval, spearman_rho, spearman_pval, rsq_model, rsq_freq, rsq_m_f, rsq_f_m, rsq_m_f_int, rsq_f_m_int, corr_freq_index, corr_freq_pval]])
...
if __name__ == "__main__":
    args=sys.argv[1:]
    if len(args) != 3:
        print("Usage: corr.py <model_id> <merged_aoa_idx_file> <output_path>")
        exit(-1)
    main(*args)
